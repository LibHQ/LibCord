...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QListWidget, QTextEdit, QLineEdit, QPushButton, QDialog, QListWidgetItem, QMessageBox, QGridLayout, QLabel, QComboBox, QProgressBar
...
class LoginScreen(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Login to LibCord")
...
class LibCord(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("LibCord")
        ...
        self.servers_list.setStyleSheet("background-color: #2F3136; color: #FFFFFF; border: none;")
        ...
        self.friends_list.setStyleSheet("background-color: #2F3136; color: #FFFFFF; border: none; font-size: 14px;")
        ...
        self.messages_text_edit.setStyleSheet("background-color: #2F3136; color: #FFFFFF; border: 1px solid #7289DA; border-radius: 5px;")
        self.right_panel_layout.addWidget(self.messages_text_edit)

        self.message_input = QLineEdit()
        self.message_input.returnPressed.connect(self.send_message)
        self.message_input.setStyleSheet("background-color: #23272A; color: #FFFFFF; border: 1px solid #7289DA; border-radius: 5px;")
    ...
    def load_data(self):
        asyncio.run(self.load_friends_and_servers())

    async def load_friends_and_servers(self):
        self.progress_bar.setRange(0, 2)
        self.progress_bar.setValue(0)
        tasks = [self.load_friends(), self.load_servers()]
                    ...
                    for channel in channels_data:
                        ...
                        if recipients:
                            ...
                            if friend_id not in existing_friends:
                                friend_info_tasks.append(self.get_friend_info(session, friend_id))
                                existing_friends.add(friend_id)
                    friends_info = await asyncio.gather(*friend_info_tasks)
                    for friend_info in friends_info:
                        item = QListWidgetItem()
                        item.setText(friend_info['username'])
                        item.setData(Qt.UserRole, friend_info['id'])
                ...
                if response.status == 200:
                    ...
                    for server in servers_data:
                        ...
                        if server_icon:
                            ...
                            pixmap = await self.load_image(session, icon_url)
                ...
                async with session.get(f"https://discord.com/api/v9/guilds/{server_id}/channels", headers=headers) as response:
                    if response.status == 200:
                        ...
                        for channel in channels_data:
                            ...
                            channel_item.setData(Qt.UserRole, channel['id'])
    ...
    async def get_friend_info(self, session, user_id):
        headers = {"authorization": f"{self.token}"}
        retry = 0
        while retry < 5:
            async with session.get(f"https://discord.com/api/v9/users/{user_id}", headers=headers) as response:
                if response.status == 200:
                    ...
                    avatar_url = f"https://cdn.discordapp.com/avatars/{user_id}/{user_data.get('avatar')}.png" if user_data.get('avatar') else ""
                    return {'id': user_id, 'username': user_data.get('username'), 'avatar_url': avatar_url}
                else:
                    retry += 1
        return {'id': user_id, 'username': 'Unknown', 'avatar_url': ''}

    async def load_image(self, session, url):
        cache_folder = os.path.join(os.path.expanduser("~"), ".libcord_cache")
